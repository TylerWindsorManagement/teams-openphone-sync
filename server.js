...
app.post('/webhook/openphone', async (req, res) => {
  try {
    ...
    if (event.type === 'call.ringing') {
      ...
      if (teamsUser && activeCall) {
        // Check if user has other active calls
        const hasOtherActiveCalls = Array.from(activeCalls.values())
          .some(call => call.userId === openPhoneUserId);
        
        if (!hasOtherActiveCalls) {
          // Set Teams status back to Available if no other active calls
          ...
          console.log(`User ${teamsUser} still has active calls, keeping busy status`);
        }
      }
    }
